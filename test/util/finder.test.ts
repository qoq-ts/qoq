...
it ('can find .js and .ts files except .d.ts', async () => {
  ...
  expect(await finder([{ pattern: ['./test/fixture/glob'] }])).not.toContain(normalizePath('z.d.ts'));
});
...
it ('can ignore file by customize', async () => {
  ...
  const matches = await finder([{ pattern: ['./test/fixture/glob'], ignore: ['**/z.js', normalizePath('y.js')] }]);
...
it ('can match empty file list', async () => {
  expect(await finder([{ pattern: ['./test/fixture/glob1'] }])).toHaveLength(0);
});
