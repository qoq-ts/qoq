...
describe('Boolean validator', () => {
  ...
  it ('may be undefined', async () => {
    ...
    expect(await validator.boolean.optional().validate(data, 'notfound')).toEqual(undefined);
  });
  ...
  it ('should has default value', async () => {
    ...
    expect(await validator.boolean.default(true).validate(newlyData, 'no-data')).toEqual(undefined);
  ...
  it ('value may be 0, 1, true and false', async () => {
    ...
    expect(await validator.boolean.validate(data, 'true1str')).toEqual(undefined);
    expect(data['true1str']).toEqual(true);

    expect(await validator.boolean.validate(data, 'trueboolean')).toEqual(undefined);
    ...
    expect(await validator.boolean.validate(data, 'false0str')).toEqual(undefined);
    expect(data['false0str']).toEqual(false);

    expect(await validator.boolean.validate(data, 'falseboolean')).toEqual(undefined);
  ...
  it ('can customize allowed values', async () => {
    expect(await validator.boolean.trueValues([5, true]).validate(data, 'id5')).toEqual(undefined);
    expect(data['id5']).toEqual(true);

    expect(await validator.boolean.trueValues(['0']).validate(data, 'false0str')).toEqual(undefined);
    expect(data['false0str']).toEqual(true);

    expect(await validator.boolean.trueValues([false]).falseValues([true]).validate(data, 'trueboolean')).toEqual(undefined);
