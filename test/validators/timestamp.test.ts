...
it('may be undefined', async () => {
  expect(await validator.timestamp.validate(data, 'microTime')).toEqual(
    undefined,
  );
  expect(
...
it('default is micro time', async () => {
  ...
  expect(await validator.timestamp.validate(data, 'unixTime')).toContain(
    'timestamp',
  );
});
