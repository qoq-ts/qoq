...
it ('may be undefined', async () => {
  ...
  expect(await validator.timestamp.optional().validate(data, 'notfound')).toEqual(undefined);
...
it ('should has default value', async () => {
  ...
  await validator.timestamp.unixTime().default(() => new Date(time)).validate(newlyData, 'time2');
...
it ('can be treated as unix time', async () => {
  expect(await validator.timestamp.unixTime().validate(data, 'unixTime')).toEqual(undefined);
...
it ('can be treated as micro time', async () => {
  expect(await validator.timestamp.milliTime().validate(data, 'microTime')).toEqual(undefined);
...
it ('can limit the minimum timestamp', async () => {
  expect(await validator.timestamp.min(new Date(data.microTime - 100)).validate(data, 'microTime')).toEqual(undefined);
  expect(await validator.timestamp.min(new Date(data.microTime + 100)).validate(data, 'microTime')).toContain('after');

  expect(await validator.timestamp.unixTime().min(new Date(data.unixTime * 1000 - 100)).validate(data, 'unixTime')).toEqual(undefined);
  expect(await validator.timestamp.unixTime().min(new Date(data.unixTime * 1000 + 100)).validate(data, 'unixTime')).toContain('after');
});

it ('can limit the maximum timestamp', async () => {
  expect(await validator.timestamp.max(new Date(data.microTime + 100)).validate(data, 'microTime')).toEqual(undefined);
  expect(await validator.timestamp.max(new Date(data.microTime - 100)).validate(data, 'microTime')).toContain('before');

  expect(await validator.timestamp.unixTime().max(new Date(data.unixTime * 1000 + 100)).validate(data, 'unixTime')).toEqual(undefined);
  expect(await validator.timestamp.unixTime().max(new Date(data.unixTime * 1000 - 100)).validate(data, 'unixTime')).toContain('before');
});
