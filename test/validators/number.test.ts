...
describe('Number validator', () => {
  ...
  it('support integer only', async () => {
    expect(
      await validator.number.onlyInteger().validate(data, 'age20_1'),
    ).toContain('must be integer');
    expect(await validator.integer.validate(data, 'age20_1')).toContain(
      'must be integer',
    );
  ...
  it('larger than minimum value', async () => {
    expect(await validator.number.min(100).validate(data, 'id2')).toContain(
      'larger than 100',
    );
    expect(
  ...
  it('smaller than maximum value', async () => {
    expect(await validator.number.max(9).validate(data, 'age10')).toContain(
      'smaller than 9',
    );
    expect(
  ...
  it('should between minimum and maximum value', async () => {
    expect(
      await validator.number.min(3).max(5).validate(data, 'id2'),
    ).toContain('between 3 and 5');
    expect(
      await validator.number.min(2, false).max(5).validate(data, 'id2'),
    ).toContain('between 2 and 5');
    expect(
      await validator.number.min(-1).max(1).validate(data, 'id2'),
    ).toContain('between -1 and 1');
    expect(
  ...
  it('should reject NAN', async () => {
    expect(await validator.number.validate(data, 'nan')).toContain(
      'must be number',
    );
  });

  it('can transform data by user', async () => {
    await validator.number.transform(String).validate(data, 'age20');
  ...
  it('can valiate or fix precision', async () => {
    expect(
      await validator.number.precision(5).validate(data, 'float3'),
    ).toBeUndefined();
    expect(
      await validator.number.precision(3).validate(data, 'float3'),
    ).toBeUndefined();
    expect(
      await validator.number.precision(2).validate(data, 'float3'),
    ).toContain('no more than');
    expect(
      await validator.number.precision(2, true).validate(data, 'float3'),
    ).toBeUndefined();
    expect(data.float3).toBe(20.33);

    expect(
      await validator.number.precision(10).validate(data, 'float10'),
    ).toBeUndefined();
    expect(
      await validator.number.precision(12).validate(data, 'float12'),
    ).toBeUndefined();
    expect(
      await validator.number.precision(11).validate(data, 'float12'),
    ).toContain('no more than');
    expect(
      await validator.number.precision(11, true).validate(data, 'float12'),
    ).toBeUndefined();
