...
describe('Number validator', () => {
  ...
  it('may be undefined', async () => {
    ...
    expect(await validator.number.optional().validate(data, 'notfound')).toBeUndefined();
  ...
  it('support integer only', async () => {
    expect(await validator.number.onlyInteger().validate(data, 'age20_1')).toContain(
      'must be integer',
    );
    expect(await validator.integer.validate(data, 'age20_1')).toContain('must be integer');

    expect(await validator.number.onlyInteger().validate(data, 'age20')).toBeUndefined();
  ...
  it('smaller than maximum value', async () => {
    ...
    expect(await validator.number.max(10, false).validate(data, 'age10')).toContain(
      'smaller than 10',
    );
    expect(await validator.number.max(10).validate(data, 'age10')).toBeUndefined();
  });
  ...
  it('should between minimum and maximum value', async () => {
    ...
    expect(await validator.number.min(2, false).max(5).validate(data, 'id2')).toContain(
      'between 2 and 5',
    );
    expect(await validator.number.min(-1).max(1).validate(data, 'id2')).toContain(
      'between -1 and 1',
    );
    expect(await validator.number.min(-1).max(2, false).validate(data, 'id2')).toContain(
      'between -1 and 2',
    );
    expect(await validator.number.min(-1).max(5).validate(data, 'id2')).toBeUndefined();
    expect(
  ...
  it('can valiate or fix precision', async () => {
    ...
    expect(await validator.number.precision(11).validate(data, 'float12')).toContain(
      'no more than',
    );
